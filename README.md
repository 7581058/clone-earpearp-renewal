...
- [x]  `<header>`, `<section>` 등 시멘틱 태그를 최대한 활용해보세요.  
        ...
        - **(추가사항)** 4월 9일 4조 피어리뷰 이후 리뷰 의견을 반영해 `aside` 태그를 추가해 HTML구조를 조금 변경 해 보았습니다.  
...
- [X]  JS가 필요한 부분 중 구현할 부분이 있다면 자유롭게 구현해보세요.(JS 과제가 아니니까 가볍게 구현하시길 추천해요)  
        - 클릭이 필요한 버튼들의 이벤트는 JS 로 구현했습니다.  
        - 1) 퀵메뉴의 전체 메뉴 나타나기, 검색창 열기   
        - 2) 우측 고정 버튼의 화면 상단으로 이동하기     
        - 메인 컨텐츠의 슬라이드를 구현하기 위해 swiper.js 를 사용했습니다.   
